...
def _oci_json(
    ...
    version: int = _oci_state.BUNDLE_VERSION,
    ...
    oci = json.dumps(
        {
            ...
            "annotations": {
                ...
                "terok.shield.version": str(_oci_state.BUNDLE_VERSION),
            },
        }
    )
...
# ── parse_dnsmasq_listen_address ─────────────────────────────────────────────


def test_parse_dnsmasq_listen_address_returns_value(tmp_path: Path) -> None:
    """parse_dnsmasq_listen_address() returns the listen-address= line value."""
    conf = tmp_path / "dnsmasq.conf"
    conf.write_text(
        f"# Generated by terok-shield\nlisten-address={KRUN_DNSMASQ_BIND}\nport=53\nbind-interfaces\n"
    )
    assert _oci_state.parse_dnsmasq_listen_address(conf) == KRUN_DNSMASQ_BIND


def test_parse_dnsmasq_listen_address_returns_none_when_absent(tmp_path: Path) -> None:
    """parse_dnsmasq_listen_address() returns None when no listen-address line is found."""
    conf = tmp_path / "dnsmasq.conf"
    conf.write_text("port=53\nbind-interfaces\n")
    assert _oci_state.parse_dnsmasq_listen_address(conf) is None


# ── _start_container_dnsmasq: runtime-aware lo IP assignment ─────────────────


def test_createruntime_skips_ip_add_for_loopback_bind(tmp_path: Path) -> None:
    """Loopback binds (default runtime) require no in-netns IP assignment.

    ``127.0.0.0/8`` is on ``lo`` by kernel default, so the hook must not
    issue an ``ip addr add`` call — only the ruleset apply and the
    dnsmasq launch.
    """
    sd = tmp_path / "sd"
    sd.mkdir()
    ...
    # Exactly two nsenter calls: ruleset apply + dnsmasq launch.  No ip addr add.
    assert mock_ns.call_count == 2
    for call in mock_ns.call_args_list:
        assert "addr" not in str(call.args), f"unexpected ip addr call: {call.args}"


def test_createruntime_adds_link_local_for_krun_bind(tmp_path: Path) -> None:
    """Non-loopback dnsmasq binds (krun runtime) get ``ip addr add`` in-netns.

    Before launching dnsmasq, the hook must assign the bind address to
    ``lo`` so ``bind-interfaces`` finds it and the krun guest's passt
    routes can reach it.
    """
    sd = tmp_path / "sd"
    sd.mkdir()
    ...
    # Three nsenter calls: ruleset apply, ip addr add, dnsmasq launch.
    assert mock_ns.call_count == 3
    ip_call = mock_ns.call_args_list[1]
    assert ip_call.args[0] == "1"  # pid
    assert ip_call.args[2:] == ("addr", "add", f"{KRUN_DNSMASQ_BIND}/32", "dev", "lo")


def test_add_local_ip_swallows_existing_address_error() -> None:
    """add_local_ip() treats the 'File exists' netlink error as success."""
    with mock.patch(
        "terok_shield.resources._oci_state.nsenter",
        side_effect=RuntimeError(
            "nsenter command failed (exit 2) cmd=('ip', 'addr', 'add'):\n"
            "RTNETLINK answers: File exists"
        ),
    ):
        # Must not raise — the address is already on lo from a prior fire.
        _oci_state.add_local_ip("1", KRUN_DNSMASQ_BIND)


def test_add_local_ip_propagates_other_errors() -> None:
    """add_local_ip() re-raises unexpected failures so callers fail-closed."""
    with mock.patch(
        "terok_shield.resources._oci_state.nsenter",
        side_effect=RuntimeError("nsenter command timed out after 30 s"),
    ):
        with pytest.raises(RuntimeError, match="timed out"):
            _oci_state.add_local_ip("1", KRUN_DNSMASQ_BIND)
